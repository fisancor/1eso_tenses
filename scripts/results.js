...
const correctAnsLogo = "correct.png"; 
const wrongAnsLogo = "wrong.png";
...
function PrintReportQuestionDetails(qstNum,question)
{
	...
	htmlHeader = "<div class='panel-heading qf-panel-question-heading-class'><b>pregunta " + textToDisplay + "</b></div>";
	textHtml += htmlHeader;
	...
	if (question.valid === 1) {
		...
		if (question.noChoice) {
			resultMsg = "Pregunta sin respuesta";
		}
		else {
	...
	//{
	//	textHtml += ". La respuesta correcta es " + GetListOfRightAnsLetters(question);
	//}
	textHtml += "</i></p>";
	...
	if (question.valid === 3)
	{
		if (question.minScore === 0) {
			if (question.typeOfQuestion === QMAKE_MATCHING) {
				textHtml += "<p class='revision-class'><i>Sentencias correctas : " + question.nScore + "/" + question.maxScore + "</i></p>";
			}
			else {
				textHtml += "<p class='revision-class'><i>Nota : " + question.nScore + "/" + question.maxScore + "</i></p>";
			}
		}
		else {
			textHtml += "<p class='revision-class'><i>Nota : " + question.nScore + " [" + question.minScore + " , " + question.maxScore + "]</i></p>";
		}
	}
	...
	$.ajax({
		...
		error: function (xhr, textStatus, errorThrown) {
			if (xhr.status === 401) {
				...
				PrintError('Error de inicio de sesión : ' + xhr.status + " - " + errorThrown, 'R');
			}
		},
		complete: function (xhr, textStatus) {
			$('html').removeClass("wait");
